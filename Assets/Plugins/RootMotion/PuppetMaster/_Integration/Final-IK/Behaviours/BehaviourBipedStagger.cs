...
namespace RootMotion.Dynamics {
	...
	public partial class BehaviourBipedStagger : BehaviourBase {
		...
		private void SolveLegs() {
			...
			if (!isFinished) {
				int nextStepLegIndex = GetNextStepLegIndex();
			...
			if (isFinished || balanceLost) weight = Mathf.MoveTowards(weight, 0f, Time.deltaTime * 5f);
		...
		private int GetNextStepLegIndex() {
			// is stepping
		...
		private void SolveArms() {
			...
			windmillAngle += Time.deltaTime * windmillSpeed.GetValue(angle) * Mathf.Rad2Deg;
		...
		protected override void OnFixedUpdate() {
			if (!balanceLost) weight = Mathf.MoveTowards(weight, 1f, Time.deltaTime * 5f);
